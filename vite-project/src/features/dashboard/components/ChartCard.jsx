...
const ChartCard = ({ title, subtitle = "Real-time Analytics" }) => {
  return (
    <div className="bg-white/60 dark:bg-[#0a1515]/40 backdrop-blur-xl border border-emerald-100 dark:border-emerald-900/30 rounded-[32px] p-6 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-200 dark:border-emerald-500/10 flex items-center justify-center text-emerald-600 dark:text-emerald-500">
            <HiOutlinePresentationChartLine size={20} /> 
          </div>
          <div>
            <h3 className="text-sm font-black text-slate-900 dark:text-white tracking-wide uppercase">{title}</h3>
            <p className="text-[10px] font-bold text-emerald-700 dark:text-emerald-500 uppercase tracking-tighter">{subtitle}</p>
          </div>
        </div>
      ...
      <div className="h-52 sm:h-64 md:h-72 w-full bg-emerald-50 dark:bg-emerald-950/10 rounded-[24px] border border-emerald-100 dark:border-emerald-500/5 flex flex-col items-center justify-center relative group-hover:border-emerald-500/20 transition-all overflow-hidden">
        ...
        <div className="text-center space-y-2 opacity-40 group-hover:opacity-100 transition-opacity duration-700">
          ...
          <p className="text-[10px] font-black text-emerald-700 dark:text-emerald-800 tracking-[4px] uppercase">
...
const ChartMiniStat = ({ label, val }) => (
  <div className="text-center">
    <p className="text-[8px] font-black text-emerald-700 dark:text-emerald-900 uppercase tracking-widest mb-1">{label}</p>
    <p className="text-xs font-bold text-emerald-600 dark:text-emerald-500">{val}</p>
  </div>
);
